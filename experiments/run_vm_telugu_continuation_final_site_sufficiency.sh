...
REMOTE_BASE="${REMOTE_BASE:-~/Research/Honors_telugu_sufficiency}"
MODEL="${MODEL:-1b}"
...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-telugu_continuation_final_site_sufficiency_v1}"
...
echo "=== Running Telugu final-site sufficiency panel on VM ==="
$SSH "cd ${REMOTE_BASE} && HF_HUB_OFFLINE=${HF_HUB_OFFLINE_REMOTE} TRANSFORMERS_OFFLINE=${TRANSFORMERS_OFFLINE_REMOTE} ${REMOTE_PY} experiments/telugu_continuation_final_site_sufficiency.py \
  --model '${MODEL}' \
  --pair '${PAIR}' \
  --seed '${SEED}' \
  --n-icl '${N_ICL}' \
