...
REMOTE_BASE="${REMOTE_BASE:-~/Research/Honors_hindi_safety}"
MODEL="${MODEL:-1b}"
...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-hindi_patch_safety_audit_v1}"
...
echo "=== Running Hindi patch safety audit on VM ==="
$SSH "cd ${REMOTE_BASE} && HF_HUB_OFFLINE=${HF_HUB_OFFLINE_REMOTE} TRANSFORMERS_OFFLINE=${TRANSFORMERS_OFFLINE_REMOTE} ${REMOTE_PY} experiments/hindi_1b_patch_safety_audit.py \
  --model '${MODEL}' \
  --pair '${PAIR}' \
  --seed '${SEED}' \
  --n-icl '${N_ICL}' \
  ...
  --min-pool-size 500 \
  --out-root 'research/results/autoresearch/${RESULTS_ROOT_NAME}/${MODEL}/seed${SEED}'"

echo "=== Downloading results ==="
mkdir -p "${RESULTS_LOCAL}/${MODEL}/seed${SEED}"
$SSH "cd ${REMOTE_BASE}/research/results/autoresearch/${RESULTS_ROOT_NAME} && tar cf - ." \
  | (cd "${RESULTS_LOCAL}" && tar xf -)

echo "=== Done ==="
ls -la "${RESULTS_LOCAL}/${MODEL}/seed${SEED}/" 2>/dev/null || true
