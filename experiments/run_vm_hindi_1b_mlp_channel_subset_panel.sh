#!/usr/bin/env bash
# Launch Hindi 1B top-negative MLP channel subset panel on the shared VM.
set -euo pipefail
...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-hindi_mlp_channel_subset_panel_v1}"
...
echo "=== Running Hindi top-negative channel subset panel on VM ==="
$SSH "cd ${REMOTE_BASE} && ${REMOTE_PY} experiments/hindi_1b_mlp_channel_subset_panel.py \
  --model ${MODEL} \
  --pair ${PAIR} \
  --seed 42 \
  --n-icl ${N_ICL} \
