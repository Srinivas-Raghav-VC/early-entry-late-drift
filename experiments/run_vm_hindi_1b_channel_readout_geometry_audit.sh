...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-hindi_channel_readout_geometry_v1}"
...
echo "=== Running Hindi channel readout geometry audit on VM ==="
$SSH "cd ${REMOTE_BASE} && ${REMOTE_PY} experiments/hindi_1b_channel_readout_geometry_audit.py \
  --model ${MODEL} \
  --pair ${PAIR} \
  --seed ${SEED} \
  --n-icl ${N_ICL} \
