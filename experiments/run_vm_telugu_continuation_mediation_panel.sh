...
REMOTE_BASE="${REMOTE_BASE:-~/Research/Honors_telugu_mediation}"
MODEL="${MODEL:-1b}"
...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-telugu_continuation_mediation_panel_v1}"
...
echo "=== Running Telugu continuation mediation panel on VM ==="
$SSH "cd ${REMOTE_BASE} && HF_HUB_OFFLINE=${HF_HUB_OFFLINE_REMOTE} TRANSFORMERS_OFFLINE=${TRANSFORMERS_OFFLINE_REMOTE} ${REMOTE_PY} experiments/telugu_continuation_mediation_panel.py \
  --model '${MODEL}' \
  --pair '${PAIR}' \
  --seed '${SEED}' \
  --n-icl '${N_ICL}' \
  ...
  --recipient '${RECIPIENT}' \
  --donor '${DONOR}' \
  --writer-sites '${WRITER_SITES}' \
  --mediator-site '${MEDIATOR_SITE}' \
