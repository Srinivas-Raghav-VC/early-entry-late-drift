#!/usr/bin/env bash
# Launch Hindi 1B selection->eval pair test on the shared VM.
set -euo pipefail
...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-hindi_mlp_channel_pair_select_eval_v1}"
...
echo "=== Running Hindi pair select->eval on VM ==="
$SSH "cd ${REMOTE_BASE} && ${REMOTE_PY} experiments/hindi_1b_mlp_channel_pair_select_eval.py \
  --model ${MODEL} \
  --pair ${PAIR} \
  --seed ${SEED} \
  --n-icl ${N_ICL} \
