...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-hindi_channel_sensitivity_audit_v1}"
...
echo "=== Running Hindi channel sensitivity audit on VM ==="
$SSH "cd ${REMOTE_BASE} && ${REMOTE_PY} experiments/hindi_1b_channel_sensitivity_audit.py \
  --model ${MODEL} \
  --pair ${PAIR} \
  --seed 42 \
  --n-icl ${N_ICL} \
