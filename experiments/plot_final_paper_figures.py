...
def _prepare_behavior_df() -> pd.DataFrame:
    agg = _read_json(PROJECT_ROOT / "research/results/autoresearch/four_lang_thesis_panel/seed_aggregate.json")
    rows = {(r["model"], r["pair"]): r for r in agg["row_aggregate"] if int(r["n_icl"]) == 64}

    mar_1b = _read_json(
        PROJECT_ROOT
        / "research/results/autoresearch/loop2_vm_controls/expansion_core64_seed42/raw/1b/aksharantar_mar_latin/nicl64/neutral_filler_recency_controls.json"
    )
    mar_4b = _read_json(
        PROJECT_ROOT
        / "research/results/autoresearch/loop2_vm_controls/expansion_core64_seed42/raw/4b/aksharantar_mar_latin/nicl64/neutral_filler_recency_controls.json"
    )
...
def _prepare_telugu_df() -> pd.DataFrame:
    med_1b = _read_json(
        PROJECT_ROOT
        / "research/results/autoresearch/telugu_continuation_mediation_v1/1b/aksharantar_tel_latin/seed42/nicl64/telugu_continuation_mediation_panel.json"
    )
    med_4b = _read_json(
        PROJECT_ROOT
        / "research/results/autoresearch/telugu_continuation_mediation_v1/4b/aksharantar_tel_latin/seed42/nicl64/telugu_continuation_mediation_panel.json"
    )
    suf_1b = _read_json(
        PROJECT_ROOT
        / "research/results/autoresearch/telugu_continuation_final_site_sufficiency_v1/1b/aksharantar_tel_latin/seed42/nicl64/telugu_continuation_final_site_sufficiency.json"
    )
    suf_4b = _read_json(
        PROJECT_ROOT
        / "research/results/autoresearch/telugu_continuation_final_site_sufficiency_v1/4b/aksharantar_tel_latin/seed42/nicl64/telugu_continuation_final_site_sufficiency.json"
    )
...
def _prepare_prompt_tradeoff_df() -> pd.DataFrame:
    ...
    for model in ["1b", "4b"]:
        payload = _read_json(
            PROJECT_ROOT / "research/results/autoresearch/prompt_composition_ablation_v1" / model / "aksharantar_tel_latin" / "seed42" / "nicl64" / "prompt_composition_ablation.json"
        )
