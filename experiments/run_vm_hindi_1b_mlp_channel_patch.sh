#!/usr/bin/env bash
# Launch Hindi 1B L25 MLP channel-basis patch experiment on the shared VM.
# Usage:
#   VM_PASS='...' bash experiments/run_vm_hindi_1b_mlp_channel_patch.sh
set -euo pipefail
...
LAYER="${LAYER:-25}"
KS="${KS:-}"
SELECTOR_POLICIES="${SELECTOR_POLICIES:-abs,pos,neg}"
RANDOM_KS="${RANDOM_KS-4,16,64,256}"
N_RANDOM="${N_RANDOM:-2}"
NO_OP_K="${NO_OP_K:-64}"
...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-hindi_mlp_channel_patch_v1}"
...
echo "=== Running Hindi L25 MLP channel patch on VM ==="
$SSH "cd ${REMOTE_BASE} && ${REMOTE_PY} experiments/hindi_1b_mlp_channel_patch.py \
  --model ${MODEL} \
  --pair ${PAIR} \
  --seed 42 \
  --n-icl ${N_ICL} \
