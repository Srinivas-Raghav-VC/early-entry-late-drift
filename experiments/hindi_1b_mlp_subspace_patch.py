...
def _register_partial_mlp_replace_hook(
    model: Any,
    layer: int,
    donor_vector: torch.Tensor,
    ...
    *,
    ...
    if layers is None:
        raise AttributeError("Could not locate model.model.layers for partial MLP replacement.")
    mlp = layers[int(layer)].mlp
    donor_vector = donor_vector.detach()
    ...
    def hook(module, inputs_tuple, output):
        ...
        idx = selected_idx.to(device=y.device)
        if idx.numel() == 0:
            return output
        idx = idx[(idx >= 0) & (idx < int(y.shape[2]))]
        if idx.numel() == 0:
            return output
        donor = donor_vector.to(device=y.device, dtype=y.dtype).view(-1)
        y_new = y.clone()
        y_new[:, pos, idx] = donor[idx].view(1, -1)
...
def _deterministic_random_indices(
    *,
    ...
    msg = f"{seed}::{label}::{hidden_size}::{k}".encode("utf-8")
    seed32 = int.from_bytes(hashlib.sha256(msg).digest()[:4], "little", signed=False)
    rng = np.random.default_rng(seed32)
    perm = rng.permutation(int(hidden_size))
    return torch.tensor(perm[: int(k)], dtype=torch.long)


def _default_k_grid(hidden_size: int) -> List[int]:
    vals = [1, 4, 16, 64, 256, 512, 1024, int(hidden_size)]
    out: List[int] = []
    for v in vals:
        vv = int(min(max(1, v), int(hidden_size)))
        if vv not in out:
            out.append(vv)
    return out
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Sparse coordinate patching for the Hindi 1B L25 MLP last-token site.")
    ...
    ap.add_argument(
        ...
        type=str,
        default="abs",
        help="Comma-separated selector policies: abs, pos, neg.",
    )
    ap.add_argument(
        "--random-ks",
        type=str,
        default="4,16,64,256,512",
        help="Subset of k values to evaluate with random-coordinate controls.",
    )
...
def main() -> int:
    ...
    k_grid = _parse_csv_int(str(args.ks)) or _default_k_grid(hidden_size)
    k_grid = [int(min(max(1, k), hidden_size)) for k in k_grid]
    k_grid = list(dict.fromkeys(k_grid))
    selector_policies = _parse_selector_policies(str(args.selector_policies))
    random_k_grid = [int(min(max(1, k), hidden_size)) for k in (_parse_csv_int(str(args.random_ks)) or [])]
    random_k_grid = [k for k in dict.fromkeys(random_k_grid) if k in set(k_grid) and k < hidden_size]
    no_op_k = int(min(max(1, int(args.no_op_k)), hidden_size))

    out_root = (
        Path(args.out).resolve()
        if str(args.out).strip()
        else PROJECT_ROOT / "research" / "results" / "autoresearch" / "hindi_mlp_subspace_patch_v1" / str(args.model) / str(args.pair) / f"nicl{int(args.n_icl)}"
    )
    out_root.mkdir(parents=True, exist_ok=True)

    print(
        f"Running Hindi L{args.layer} MLP subspace patch: model={args.model} pair={args.pair} select_items={len(select_rows)} eval_items={len(eval_rows)} selector_policies={selector_policies} k_grid={k_grid} random_k_grid={random_k_grid}",
        flush=True,
    )

    # ------------------------------------------------------------------
    # Selection split: rank coordinates by mean absolute zs-vs-helpful delta.
    # ------------------------------------------------------------------
    abs_delta_sum = torch.zeros(hidden_size, dtype=torch.float32)
    signed_delta_sum = torch.zeros(hidden_size, dtype=torch.float32)
    sign_agreement_sum = torch.zeros(hidden_size, dtype=torch.float32)
    n_selector_items = 0
    ...
    for idx, word in enumerate(select_rows, start=1):
        ...
        zs_vec = _extract_mlp_io_at_position_from_input_ids(
            model,
            cond_inputs["zs"]["input_ids"],
            int(args.layer),
            int(cond_inputs["zs"]["last_position"]),
        )[1].detach().float().cpu()
        helpful_vec = _extract_mlp_io_at_position_from_input_ids(
            model,
            cond_inputs["icl_helpful"]["input_ids"],
            int(args.layer),
            int(cond_inputs["icl_helpful"]["last_position"]),
        )[1].detach().float().cpu()
        delta = zs_vec - helpful_vec
        abs_delta_sum += torch.abs(delta)
        signed_delta_sum += delta
        sign_agreement_sum += torch.sign(delta) * torch.sign(torch.mean(delta))
        n_selector_items += 1
    ...
    sign_consistency = torch.zeros_like(mean_signed_delta)
    nonzero = mean_signed_delta != 0
    sign_consistency[nonzero] = torch.sign(mean_signed_delta[nonzero]) * (signed_delta_sum[nonzero] / torch.clamp(abs_delta_sum[nonzero], min=1e-8))
            ...
            rows.append(
                {
                    ...
                    "sign_consistency": float(sign_consistency[int(dim)].item()),
                }
            )
        selector_rank_rows_by_policy[policy_name] = rows

    # ------------------------------------------------------------------
    # Evaluation split: selected-vs-random coordinate patching on held-out rows.
    # ------------------------------------------------------------------
    ...
    for item_idx, word in enumerate(eval_rows, start=1):
        ...
        donor_vec_by_condition: Dict[str, torch.Tensor] = {}
        for condition in CONDITIONS:
            ...
            donor_vec_by_condition[condition] = _extract_mlp_io_at_position_from_input_ids(
                model,
                input_ids,
                int(args.layer),
        ...
        for recipient_condition, donor_condition in PATCH_DIRECTIONS:
            ...
            donor_vec = donor_vec_by_condition[donor_condition]
            ...
            for selector_policy in selector_policies:
                ...
                for k in k_grid:
                    ...
                    if idx_selected.numel() <= 0:
                        continue
                    hook = _register_partial_mlp_replace_hook(
                        model,
                        int(args.layer),
                        donor_vec,
                        idx_selected,
                    ...
                    interventions.append(
                        {
                            ...
                            "random_repeat": None,
                            "selected_dims": [int(x) for x in idx_selected.detach().cpu().tolist()],
                            "base": base_stats,
                            "patched": patched,
                            "delta": {
                    ...
                    interventions.append(
                        {
                            ...
                            "selected_dims": [int(x) for x in idx_random.detach().cpu().tolist()],
                            "base": base_stats,
                            "patched": patched,
                            "delta": {
                                "target_prob": float(patched["target_prob"] - base_stats["target_prob"]),
            ...
            self_vec = donor_vec_by_condition[recipient_condition]
            hook = _register_partial_mlp_replace_hook(
                model,
                int(args.layer),
                self_vec,
            ...
            interventions.append(
                {
                    ...
                    "random_repeat": None,
                    "selected_dims": [int(x) for x in idx_noop.detach().cpu().tolist()],
                    "base": base_stats,
                    "patched": patched,
                    "delta": {
                        "target_prob": float(patched["target_prob"] - base_stats["target_prob"]),
                        ...
                        for item in item_rows:
                            for intr in item["interventions"]:
                                if (
                                    ...
                                    and intr["random_repeat"] == repeat_id
                                ):
    ...
    # Aggregate random rows to simplify comparison at read time.
    ...
    payload = {
        "experiment": "hindi_1b_mlp_subspace_patch",
        ...
        "selector": {
            ...
            "score": "mean_abs_delta(zs_mlp_out_last_token - icl_helpful_mlp_out_last_token)",
            "requested_policies": list(selector_policies),
            "interpretation_note": "Absolute ranking is used for the main concentration test; positive-only and negative-only policies keep only coordinates with signed mean delta > 0 or < 0 on the selection split.",
            "top_ranked_dims_by_policy": selector_rank_rows_by_policy,
        },
        "oracle": {
            "description": "Held-out eval test of whether a small L25 MLP coordinate subset carries a disproportionate share of the Hindi target-vs-Latin competition effect.",
            "primary_hypothesis": "On eval, selected top-k coordinates should beat matched random-coordinate controls on delta_mean_gap_latin for helpful<-zs patching.",
            "secondary_hypotheses": [
                "The selected k-curve should approach the dense-patch effect as k grows.",
                "The same selected coordinates should produce opposite-sign degradation under zs<-icl_helpful.",
                "Self-patching at the same coordinates should be a no-op sanity check.",
            ],
        },
    ...
    out_path = out_root / "hindi_1b_mlp_subspace_patch.json"
    _write_json(out_path, payload)
    print(f"Saved: {out_path}", flush=True)

    print("\n=== Helpful<-zs selected-topk curve ===", flush=True)
    for selector_policy in selector_policies:
        ...
        for row in helpful_rows:
            rand_str = "NA" if row["random_mean_delta_gap_latin"] is None else f"{row['random_mean_delta_gap_latin']:.3f}"
            frac_str = "NA" if row["fraction_of_dense_gap_latin"] is None else f"{row['fraction_of_dense_gap_latin']:.3f}"
            actual_k = int(row.get("actual_k", row["k"]))
            print(
                f"  k={row['k']:>4d} actual_k={actual_k:>4d} selected_dgapLatin={row['selected_delta_mean_gap_latin']:.3f} random_mean={rand_str} frac_dense={frac_str}",
                flush=True,
            )
    return 0
