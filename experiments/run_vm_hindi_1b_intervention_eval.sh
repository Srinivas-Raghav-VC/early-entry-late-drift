...
RESULTS_ROOT_NAME="${RESULTS_ROOT_NAME:-hindi_intervention_eval_v1}"
...
echo "=== Running Hindi intervention eval on VM ==="
$SSH "cd ${REMOTE_BASE} && HF_HUB_OFFLINE=${HF_HUB_OFFLINE_REMOTE} TRANSFORMERS_OFFLINE=${TRANSFORMERS_OFFLINE_REMOTE} ${REMOTE_PY} experiments/hindi_1b_intervention_eval.py \
  --model '${MODEL}' \
  --pair '${PAIR}' \
  --seed '${SEED}' \
  --n-icl '${N_ICL}' \
